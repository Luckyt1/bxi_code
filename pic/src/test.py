...
class WristControlNode(Node):
    ...
    def process_and_publish(self):
        ...
        try:
            ...
            # 创建完整的力矩数组：前7个是左臂重力补偿，后11个补零
            torque_array = np.zeros(18, dtype=np.float32)
            torque_array[:7] = tau * direction  # 确保是float32类型
            print(torque_array)
    ...
    def joint_state_callback(self, msg):
        ...
        try:
            ...
            # 在22和31位置后各补一个0
            self.received_joint_pos = np.insert(pos, [22, 31], [0, 0])
            # print(f"接收到的关节位置（33位）：{self.received_joint_pos}")
            # 添加7个freejoint参数（baselink的位置和姿态）
            prefix_values = np.array([0, 0, 0, 1, 0, 0, 0], dtype=np.float32)
            self.compensator.qpos = np.concatenate([prefix_values, self.received_joint_pos])
