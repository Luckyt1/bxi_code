...
int SpeechProcess::init_asr_params(){
    ...
    cout << ">>>>>ASR resource path: " << jet_path << endl;
    cout << ">>>>>Grammar build path: " << grammer_path << endl;
    cout << ">>>>>BNF file path: " << bnf_path << endl;
    cout << ">>>>>Denoise sound path: " << denoise_sound_path << endl;
    ...
    {
        cout << ">>>>>初始化离线资源参数失败，请检查:" << endl;
        cout << "  1. ASR资源路径是否正确" << endl;
        cout << "  2. 语法文件(.bnf)是否存在" << endl;
        cout << "  3. 讯飞SDK库文件是否完整" << endl;
        return -1;
    }
    return 0;
}
...
: rclcpp::Node(node_name){
	...
	{
		RCLCPP_ERROR(this->get_logger(),"Failed to initialize ASR parameters!");
		return;
	}

	// 延迟启动线程，确保所有 ROS 2 资源已完全初始化
